						  ...
						  std::function<bool(std::unique_ptr<graph_state>&)> callback, bool print_stats)
{
	...
	{
		auto stop_time = std::chrono::high_resolution_clock::now();
		std::cout << "elapsed time: " << parse_time(std::chrono::duration_cast<std::chrono::milliseconds>(stop_time - start_time).count()) << std::endl;
		std::cout << "#closed: " << closed.size() << ", #open: " << open.size() << std::endl;
	}
}
...
std::string informative_searcher::get_stats() const
{
