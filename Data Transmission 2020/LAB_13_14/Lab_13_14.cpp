...
double N = 2.0, A1 = 1.0, A2 = 2.0, Tb = 1.0, f = 100.0;
double fs = 2;
double fi = B;
double Amp = 1.0;
int q = 16;
...
public:
	...
	Sygnal operator*(double alfa) const {
		Sygnal nowy;
		nowy.ymin = 0;
		nowy.ymax = 0;
		nowy.x = this->x;
		for (int i = 0; i < this->x.size(); i++) {
			nowy.y.push_back(this->y[i] * alfa);
			if (nowy.ymin > nowy.y[i])
				nowy.ymin = nowy.y[i];
			if (nowy.ymax < nowy.y[i])
				nowy.ymax = nowy.y[i];
...
vector<double> Szum(int numSamples) {
	srand(time(NULL));
...
Sygnal DodajSzum(Sygnal in, vector<double> szum, double alfa) {
	Sygnal out = in;
	in = in * alfa;
	out.y = szum;
	out = out * (1-alfa);
...
double BER(vector<double> original, vector<double> decoded) {
	double errorCounter = 0;
	for (int i = 0; i < original.size(); i++) {
		if (original[i] != decoded[i])
			errorCounter++;
	}
	return double(errorCounter / double(original.size()));
}
...
int main() {
	...
	vector<double> szum = Szum(prost.y.size());
	...
	double alfa = 0.7;
	//vector<Zespolona> dft=DFT(za1.y);
	//vector<double> widmo = oblicz_widmo(dft);
	/*rysuj_amplitudowy("Widmo bez szumu.(ASK)", za1.y.size(), f, widmo);
	dft = DFT(zp1.y);
	widmo = oblicz_widmo(dft);
	rysuj_amplitudowy("Widmo bez szumu.(PSK)", zp1.y.size(), f, widmo);
	dft = DFT(zf1.y);
	widmo = oblicz_widmo(dft);
	rysuj_amplitudowy("Widmo bez szumu.(FSK)", zf1.y.size(), f, widmo);*/
	za1 = DodajSzum(za1, szum, alfa);
	//dft = DFT(za1.y);
	//widmo = oblicz_widmo(dft);
	//rysuj_amplitudowy("Widmo z szumem.(ASK,alfa-70%)", za1.y.size(), f, widmo);
	zp1 = DodajSzum(zp1, szum, alfa);
	//dft = DFT(zp1.y);
	//widmo = oblicz_widmo(dft);
	//rysuj_amplitudowy("Widmo z szumem.(PSK,alfa-70%)", za1.y.size(), f, widmo);
	zf1 = DodajSzum(zf1, szum, alfa);
	//dft = DFT(zf1.y);
	//widmo = oblicz_widmo(dft);
	//rysuj_amplitudowy("Widmo z szumem.(FSK,alfa-70%)", za1.y.size(), f, widmo);
	Sygnal xa = za1 * sa;
	Sygnal xp = zp1 * sp;
	Sygnal xf1 = zf1 * sf1;
	Sygnal xf2 = zf1 * sf2;
	...
	Sygnal ma = WyliczM(pa, 0.01);
	Sygnal mp = WyliczM(pp, 0.0001);
	Sygnal mf = WyliczM(pf, 0.005);
	...
	*/
	...
	cout << "Wspolczynnik BER ASK: " << BER(out, DecHamA) << endl;
	...
	cout << "Wspolczynnik BER PSK: " << BER(out, DecHamP) << endl;
	...
	cout << "Wspolczynnik BER FSK: " << BER(out, DecHamFs) << endl;
}